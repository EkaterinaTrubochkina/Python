...
def list_of_line(text):
    ...
    for i in k:
        if not i.isalpha():
            parts = i.lstrip().split('.')
            parts = [item.replace(item, item.lstrip().capitalize()) for item in parts]
            t = '.'.join(parts)
            m.append(t)
        else:
            m.append(t)
...
if __name__ == '__main__':
    print('\n' + 'HomeWork_3:')
    correction_iz(list_of_line(init_text))
    new_line_creating(new)
    print(text_from_list(new))
